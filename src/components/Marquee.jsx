...
function Marquee({ phrases, className }) {
    const duplicatedPhrases = phrases && phrases.length > 0 ? [...phrases, ...phrases, ...phrases] : [['']]; // empty array if no phrase is provided, because it's still going to display the div
    
	return (
		<div className={classNames(styles.marquee, className)}>
			<div className={styles.marquee_content}>
				{duplicatedPhrases && (
					duplicatedPhrases.map((phrase) => {
						return (
							<>
